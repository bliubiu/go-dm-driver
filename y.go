...
/**
 * dm_svc.conf中配置的服务名对应的一组实�? 以及相关属性和状态信�?
 *
 * 需求：
 * 1. 连接均匀分布在各个节点上
 * 2. loginMode，loginStatus匹配
 * 3. 连接异常节点比较耗时，在DB列表中包含异常节点时异常连接尽量靠后，减少对建连接速度的影�?
 *
 *
 * DB 连接顺序�?
 * 1. well distribution，每次连接都从列表的下一个节点开�?
 * 2. 用DB sort值按从大到小排序，sort为一个四位数XXXX，个�?-serverStatus，十�?-serverMode，共 有三种模式，最优先�?*100, 次优先的*10
 */
type epGroup struct {
	name       string
	epList     []*ep
...
func (g *epGroup) connect(connector *DmConnector) (*DmConnection, error) {
	...
	var ex error = nil
	// 如果配置了loginMode的主、备等优先策略，而未找到最高优先级的节点时持续循环switchtimes次，如果最终还是没有找到最高优先级则选择次优先级�?
...
*
* @exception
* DBError.ECJDBC_INVALID_SERVER_MODE 有站点的模式不匹�?
* DBError.ECJDBC_COMMUNITION_ERROR 所有站点都连不�?
 */
